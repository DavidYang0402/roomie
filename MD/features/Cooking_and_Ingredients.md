...
**第一輪修正(上一次回報時做的)**:原本的份數輸入框巢在「checkbox 的 `<label>` 裡面」,除了造成
版面擠壓外,更根本的問題是點擊有時會被瀏覽器的 label 點擊轉發機制誤判成切換 checkbox。改成獨立的
`.ing-pick-row`/`.ing-pick-main`/`.ing-pick-text` class(不共用分帳畫面的 `.chk`),qty-input
變成同一列的 sibling、不再巢在 label 裡。**但這輪你回報版面問題仍然存在**(附截圖:名稱被擠成
直排、輸入框蓋住文字)。

**這次找到真正的根因,不是靠猜**:用 `getBoundingClientRect()` 量測實際渲染結果,發現
`.ing-pick-main`(裝 checkbox+名稱的容器)寬度是 **0px**、高度卻有 116~147px(文字被擠到一個字
一行,直排顯示);`.qty-input` 寬度則是 **310px**(遠超設定的 64px,幾乎佔滿整列)——這代表
`.qty-input` 的 `width: 64px` 完全沒生效。

追下去發現這個區塊(「要用的食材」)包在既有的 `.field` 容器裡(跟菜名/日期共用欄位間距),而
`.field` 底下有一條全域規則 `.field input:not([type="checkbox"])  { width: 100%; ... }`
(用來讓一般表單欄位撐滿寬度)。`.field` 用的是「後代選擇器」,不是只認直接子層,所以會一路套用到
巢在裡面的 qty-input;而且這條規則多了 `:not([type="checkbox"])`,CSS specificity 是
`(0,2,1)`,比我原本寫的 `.ing-pick-row .qty-input`(`(0,2,0)`)還高一級,所以完全蓋掉了我想要的
64px——這就是「數量輸入框蓋到文字上面」的真正成因。checkbox 沒事,是因為那條規則的 `:not(...)`
本來就排除了 checkbox。

**修正**:把選擇器改成 `.modal .ing-pick-row .qty-input`(`(0,3,0)`),specificity 明確蓋過
`.field input:not(...)`,不用 `!important`。另外 `.ing-pick-row` 加上 `flex-wrap: wrap` +
`.ing-pick-main { flex: 1 1 180px }`,讓真的太窄時可以自然換行,而不是硬擠。

**已實測(375px 手機寬度 + 1280px 桌面寬度,測試名稱含短名「高麗菜」與長名「奇異果切片」)**:
兩種寬度下,`.ing-pick-main` 高度都是 65px(單行,不再直排)、名稱寬度正常(44px / 74px)、
checkbox 18×18、`.qty-input` 正確等於 64px、文字與輸入框無重疊(以上皆用
`getBoundingClientRect()` 精確量測,不是目測)。⚠️ **這次沒有附上實際截圖**——瀏覽器的
screenshot 工具這個 session 持續逾時失敗(navigate/DOM 檢查等其他操作都正常,判斷是這次 session
的環境層級問題,不是頁面卡住),試了多次(mobile、desktop 都試過)都無法取得圖片,已如實告知你這個
限制,沒有假裝有截圖。改用逐像素的 `getBoundingClientRect()` 量測結果作為證據,精確度不亞於截圖。

---

## Post-Release IA 改版(2026-07-12,使用回饋)

> 先出設計草圖(用 mockup widget)給你看過、確認方向後才實作。3 項需求皆已完成並在瀏覽器實測。

### 1. 排菜彈窗:食材清單過長時內部捲動,標題與底部按鈕固定 —— ✅ 已完成,已實測
- CSS(`src/index.css`):`.ing-pick-list` 加 `max-height: min(46vh, 340px)` + `overflow-y: auto`
  (用 vh 讓上限隨螢幕高度縮放);另把 `.modal` 改成 flex column + `max-height: calc(100dvh - 8px)`
  + `.modal-body { overflow-y: auto }` 當保險,確保任何彈窗都不會超出畫面把按鈕擠出去。無需 migration。
- **已實測**(seed 11 個食材撐爆清單):清單高度被限制在 322px(手機)/340px(桌面)、內部可捲動
  (scrollHeight 798 > clientHeight);捲動清單時,rows 位移(firstRow 267→-133)但 `.modal-head`
  (top 28)與 `.modal-actions`(top 653)**完全不動**——正是你要的「清單捲、頭尾固定」。
- ⚠️ 量測時發現一個**測試環境假象**(非真實 bug):彈窗的開場動畫 `sheet`(`translateY(16px)` 滑入)
  在這個 headless preview 裡卡在第一幀不前進(`transform` 停在 `matrix(1,0,0,1,0,16)`),那 16px 位移
  讓彈窗底部在量測當下超出畫面 16px。把 transform 清掉(模擬動畫跑完)後重量,彈窗完美落在畫面內
  (top 8 / bottom 701,按鈕完整可見)。真實瀏覽器 0.18s 內動畫會跑完、transform 歸零,不會有這問題,
  所以**沒有為了這個測試環境假象去改動畫或版面**(那樣反而會讓真實瀏覽器變錯)。

### 2. 「食材」/「煮菜規劃」改用子頁籤切換 —— ✅ 已完成,已實測
- `src/components/Cooking.tsx`:新增 `sub` 狀態(`'ingredients' | 'dishes'`),在「煮菜」h1 下方放一排
  **沿用既有 `Segmented` 元件**的子頁籤(不另造樣式,語意上就是「切換同頁多個檢視」,跟待辦頁的
  「全部/我的/待認領」同一個元件)。頂部 `view-head` 的動作按鈕跟著子頁切換:食材頁顯示「＋ 新增食材」、
  煮菜規劃頁顯示「＋ 排菜」。CSS 只加了一條 `.subtabs { margin-bottom: 16px }`。
- **已實測**:切到「食材」只顯示食材清單 + 新增食材鈕;切到「煮菜規劃」只顯示菜色清單 + 排菜鈕,
  食材清單不再垂直堆疊佔位。console 無錯誤。

### 3. 版面:手機與桌面都用橫向子頁籤(採你確認的建議,不做桌面左右並排)—— ✅ 已完成,已實測
- **已實測**:375px(手機)子頁籤橫排、底部導覽列顯示;1280px(桌面)子頁籤同樣橫排、頂部導覽列顯示、
  底部導覽列隱藏、內容維持 720px 置中單欄。長食材名「紅蘿蔔切片」(74px)在勾選清單裡與 qty-input(64px)
  無重疊、單行 65px 高,上一輪的版面修正在新的捲動清單裡仍然成立。
- ⚠️ 同 §Post-Release Fixes:screenshot 工具這個 session 仍持續逾時,依先前跟你的約定「先試截圖、
  拿不到就用量測數據交付,不假裝有截圖」,以上皆為 `getBoundingClientRect()` 精確量測結果。

`npx tsc -b`、`npm test`(6/6)、`npm run build` 均通過。測試用 seed 的 10 個食材已於測試後清除,
未留在資料庫。
...
| 2026-07-12 | 4 點回饋均認可;§4 排菜版面問題你回報上一輪沒真的修好,附截圖;重新找出真正根因並修正,已用精確量測驗證(截圖工具本身故障,如實告知) | 用 `getBoundingClientRect()` 實測(不是猜)找到真因:`.qty-input` 的 `width:64px` 被 `.field input:not([type="checkbox"])`(specificity `(0,2,1)`)蓋過,因為我原本寫的 `.ing-pick-row .qty-input` 只有 `(0,2,0)`,少一級;checkbox 沒事是因為那條規則排除了 checkbox。改成 `.modal .ing-pick-row .qty-input`(`(0,3,0)`)明確蓋過,不用 `!important`;另加 `flex-wrap:wrap` 讓極窄情況能自然換行。用 375px(手機)與 1280px(桌面)、短名「高麗菜」與長名「奇異果切片」四種組合實測,量測結果:容器高度從 116~147px(直排 bug)恢復到 65px(單行),qty-input 從 310px 恢復到 64px,無重疊。過程中誤用 `javascript_exec` 直接觸發一個 `confirm()` 對話框卡死瀏覽器分頁(該工具的 dialog 自動處理似乎只認得真人模擬點擊,不認得程式化 `.click()` 觸發的對話框),開新分頁後改用 stub `window.confirm` 的方式清掉測試資料,已記取教訓。**screenshot/zoom 工具這個 session 持續逾時失敗**(其他操作都正常),試了兩種寬度都無法取得圖片,已如實告知你這個限制,沒有假裝附上截圖,改附精確到 px 的量測數據作為證據。`npm test`(6/6)、`npm run build` 均通過。 |